 ...
 * PresentationDock
 * ─────────────────────────────────────────────────────────────────────────────
 * - Clicking a thumbnail lifts the photo IN FRONT of the 3D planets
 * - The dock below it collapses smoothly to just the header bar
 * - ESC or click the photo again to dismiss and re-expand the dock
 */
export function PresentationDock({
  ...
  const [showGallery, setShowGallery]       = useState(false);   // inline col open
  const [liftedImage, setLiftedImage]       = useState(null);    // photo floating above planets
  ...
  // Open lifted photo view
  const openLifted = (item, idx) => {
  ...
  return (
    <>
      {/* ═══════════════════════════════════════════════════════
          LIFTED PHOTO — floats above 3D canvas, behind dock
          ═══════════════════════════════════════════════════════ */}
      {liftedImage && (
        <div
          className="lifted-photo-backdrop"
          onClick={closeLifted}
        >
          <div className="lifted-photo-frame" onClick={(e) => e.stopPropagation()}>
            <img
              src={liftedImage.url}
              alt={liftedImage.title}
              className="lifted-photo-img"
              onError={(e) => {
      ...
      {/* ═══════════════════════════════════════════════════════
          PRESENTATION DOCK — collapses when photo is lifted
          ═══════════════════════════════════════════════════════ */}
      <div className={`bottom-presentation-dock ${isSecretLove ? 'secret-love-theme' : ''} ${liftedImage ? 'dock-collapsed' : ''}`}>
        ...
        <div className="dock-header-bar">
          ...
          <div className="dock-actions-row">
            ...
            <button className="dock-nav-btn icon-only" onClick={handlePrev}
              disabled={currentStageIndex === 0}>◀</button>
            <button className="dock-nav-btn icon-only" onClick={handleNext}
              disabled={currentStageIndex === stages.length - 1}>▶</button>
        ...
        <div className="dock-body-wrap" ref={bodyRef}>
          <div className={`dock-body-grid ${showGallery ? 'gallery-mode' : ''}`}>
            ...
            {/* Right col: inline gallery photo viewer */}
              ...
              <div className="dock-col dock-col-gallery">
                <div className="inline-gallery-stage">
                  ...
                  <div className="inline-gallery-img-wrap">
                    <img
                      key={currentImage.url + activeImageIndex}
                      src={currentImage.url}
                      alt={currentImage.title}
                      className="inline-gallery-img"
                      onClick={() => openLifted(currentImage, activeImageIndex)}
                      style={{ cursor: 'zoom-in' }}
                      title="Click to expand above planets"
                      onError={(e) => {
                        e.currentTarget.style.display = 'none';
                        e.currentTarget.nextSibling.style.display = 'flex';
                      }}
